#' The toolbox for analysing single-cell protein intensity from the LEGENDScreen assay - imputation with MICE framework
#'
#' @description This function is an end-to-end toolbox for analysing the data from the LEGENDScreen assay in FCS format, including data normalisation, imputation (MICE) and cluster analysis.
#'
...
function(FCSpath,
  ...
  mkImputeMT(paths)
    ...
    ##5-1.imputation_bkb.pred
    imputation_bkb.predictors(paths,
               chans,
               yvar=yvar,
               cores=cores,
               models.use = "LM",
               extra_args_regression_params = list(degree = 1),
