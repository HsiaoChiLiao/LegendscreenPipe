...
function(paths,
                               ...
                               control.wells)
  {
    ...
    for(i in 1:length(preds)){
      message(paste0("Cluster analysis for ", names(preds)[i], "..."))
      impu.mt <- preds[[i]]
      ...
      message("Running UMAP...")
      a <- Sys.time()
      umap.bkb.impuPE <- umap(complete.dat,
                         n_neighbors = 15, min_dist = 0.2, metric = "euclidean", n_epochs = 2000)
      b <- Sys.time()
      b-a #11.9143 mins
      saveRDS(umap.bkb.impuPE, file = file.path(paths["downstream"], paste0("ClusterAnalysis_", 
                                                           names(preds)[i], "_umap_", 
                                                           length(bkb.v),"bkb.", (ncol(complete.dat)-length(bkb.v)),"impuPE.rds")))
      
      message("Running Phenograph...")
      a <- Sys.time()
      phenog.bkb.impuPE <- Rphenograph(complete.dat, 
                                  k = 50)  #knn_fun = "hnsw", 
      b <- Sys.time()
      b-a #17.67467 mins
      saveRDS(phenog.bkb.impuPE, file = file.path(paths["intermediary"], paste0("ClusterAnalysis_", names(preds)[i], "_phenog_",
                                                             length(bkb.v),"bkb.", (ncol(complete.dat)-length(bkb.v)),"impuPE.rds")))
    
      if(i == 1){
        metadata.cell[which(metadata.cell$train_set == 0),paste0("GP.denoised.bkb")] <- as.factor(membership(phenog.bkb[[2]]))
      }
      metadata.cell[which(metadata.cell$train_set == 0),paste0("GP.denoised.bkb.impuPE_", names(preds)[i])] <- as.factor(membership(phenog.bkb.impuPE[[2]]))
      ...
      message("\tVisualising clusters...")
      {
        ...
        ## bkb
        {
          if(i == 1){ ## would not be affected by imputation method
            graph.dat <- data.frame(umap.bkb, metadata.cell[which(metadata.cell$train_set == 0),
                                                            paste0("GP.denoised.bkb")])
        ...
        ## bkb+impuPE
        {
          graph.dat <- data.frame(umap.bkb.impuPE, metadata.cell[which(metadata.cell$train_set == 0),
                                                                 paste0("GP.denoised.bkb.impuPE_",names(preds)[i])])
          colnames(graph.dat) <- c("UMAP1", "UMAP2", "Cluster")
          ...
          jpeg(file = file.path(paths["graph"], paste0("/ClusterStructure_UMAP_", names(preds)[i], length(bkb.v), "bkb",
                                (ncol(complete.dat)-length(bkb.v)),"impuPE_colPhenog_600x750.jpeg")), height = 600, width = 750, res = 80)
          p <- ggplot(graph.dat, aes(UMAP1, UMAP2, colour=Cluster)) + geom_point(size = 0.05, alpha = 0.25) + 
            labs(titles = paste0(names(preds)[i], " ", length(bkb.v), "bkb. ", (ncol(complete.dat)-length(bkb.v)),"impuPE \n(", nrow(graph.dat), " cells)")) +
            guides(colour = guide_legend(override.aes = list(size=8))) + theme_bw() + 
            theme(text = element_text(size=22)) + scale_color_manual(values=col.coeff[1:n])
          print(p)
          dev.off()
